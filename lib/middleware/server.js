...
/**
 * Just copy code from hexo-server/lib/server.js, if there is any problem please contact me.
 * */
...
module.exports = function(args) {
  ...
  const ip = args.i || args.ip || config_port || config.webhook.ip || undefined;
  const port = parseInt(args.p || args.port || config.webhook.port || process.env.port, 10) || 4000;
  const { root } = config;
